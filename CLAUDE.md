...
- New hook script → add to `templates/hooks/` AND to `templates/gitignore-additions` AND to the hook wiring in `templates/settings.local.json` AND to the `bootstrap.sh` hook loop AND as a multi-select option in `skills/bootstrap-workflow/SKILL.md` step 3 (with a sensible pre-check rule based on stack detection) AND a one-line entry in the `HOOKS_BLOCK` builder inside `bootstrap.sh` so it shows up under `Hooks installed` in `CLAUDE.local.md`. Declare which stacks the hook applies to and filter accordingly.
