...
for pattern in "${DANGEROUS_PATTERNS[@]}"; do
  if echo "$COMMAND" | grep -qE "$pattern"; then
    echo "BLOCKED: '$COMMAND' matches dangerous pattern '$pattern'. Refused by dominikwozniak-skills guardrail. If you genuinely need this, the user must run it manually." >&2
    exit 2
  fi
done
